...
$.getJSON("recursos/json/MX09017AGNCL01FO001AYSE001AP8.json", function(data){
  primerDropdown += '<option value="">Select country</option>';
...
$("#nivel1").change(function () {
  ...
  if ($dropdown != "") {
    $.getJSON('subfondos.json', function(data){
      segundoDropdown += '<option value="">Favor de seleccionar un subfondo</option>';
      $.each(data, function(key, value){
        if($dropdown == value.yo)
        {
          segundoDropdown += '<option value="'+value.yo+'">'+value.TITULO+'</option>';
        }
      });
...
$("#nivel2").change(function () {
  ...
  if ($dropdown != "") {
    $.getJSON('subfondos.json', function(data){
      tercerDropdown += '<option value="">Favor de seleccionar un expediente</option>';
      $.each(data, function(key, value){
        if($dropdown == value.yo)
        {
          tercerDropdown += '<option value="'+value.yo+'">'+value.TITULO+'</option>';
        }
      });
      $('#nivel3').html(segundoDropdown);
